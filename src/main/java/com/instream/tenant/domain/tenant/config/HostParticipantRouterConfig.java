...
@Configuration
public class HostParticipantRouterConfig extends RouterConfig {
    private final String v1HostParticipantRoutesTag = "v1-host-participant-routes";

    @Autowired
    public HostParticipantRouterConfig(ObjectMapper objectMapper) {
        super(objectMapper);
    }
    ...
    public RouterFunction<ServerResponse> v1HostParticipantRoutes(ParticipantHandler participantHandler) {
        return route().nest(RequestPredicates.path("/v1/hosts/{hostId}/participants"),
                ...
                ops -> ops.operationId("v1HostParticipantRoutes")
        ).build();
    }
    ...
    private void buildSearchParticipantsSwagger(Builder ops) {
        ...
        ops.operationId(String.format("pagination_%s", ParticipantJoinDto.class.getSimpleName()))
                .summary("해당 테넌트의 참가자 세션 검색 API")
                .description("""
                        해당 테넌트의 참가자 세션을 검색합니다.
