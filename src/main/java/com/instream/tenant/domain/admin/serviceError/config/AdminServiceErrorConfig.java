...
public class AdminServiceErrorConfig {
    ...
    private RouterFunction<ServerResponse> getServiceErrorDetail(AdminServiceErrorHandler adminServiceErrorHandler) {
        return route()
            .GET(
                "/{errorId}",
                adminServiceErrorHandler::getServiceErrorDetail,
                ops->ops.operationId("919")
                    .parameter(parameterBuilder().name("errorId").in(ParameterIn.PATH).required(true).example("1"))
